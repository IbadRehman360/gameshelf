...
export function NotLoggedInCta({ onToggle }) {
  return (
    <div
      className=' bg-slate-50 border-t py-4 fixed z-10 bottom-0 left-0 w-full shadow-xl flex justify-center items-center gap-4 text-sm font-light 
        sm:text-xs sm:bg-transparent sm:max-w-fit sm:border-none sm:static sm:py-0 sm:z-0 sm:shadow-none sm:gap-1
        lg:gap-6 lg:text-sm'
    >
      <Link
        onClick={onToggle}
        className='border border-black py-2 w-[40%] text-center rounded text-black hover:bg-navy-blue hover:text-white
      ...
      <Link
        to='/login'
        className='border border-black py-2 w-[40%] text-center rounded bg-navy-blue hover:bg-transparent hover:text-black
